...
class KNearestNeighbor(object):
  ...
  metric = 'Euclidean'
  def __init__(self, dist_m='Euclidean'):
    self.metric = dist_m
    ...
    for i in xrange(num_test):
      for j in xrange(num_train):
        ...
        if dist_metric =='Euclidean':
          dists[i, j]= distance_euclidean(X[i, :,:], self.X_train[j,:, :])
        if dist_metric =='Riemmanian':
            dists[i, j]= distance_riemann(X[i, :,:], self.X_train[j,:, :])
        #####################################################################
        #                       END OF YOUR CODE                            #
        #####################################################################
    return dists
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in xrange(num_test):
      ...
      #######################################################################
      # dists[i, :]= (np.sqrt((X[i, :]-self.X_train.transpose()) * 2)).sum(axis=1)  
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      index = np.argsort(dists[i,:]) #1 - first dimension
